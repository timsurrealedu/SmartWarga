...
export const LandingView = ({ onStart, onLogin, onLoginUser, theme, setTheme }: LandingViewProps) => {
  return (
    <div className="min-h-screen bg-canvas text-text-main overflow-x-hidden">
      {/* Navigation */}
      <nav className="fixed top-0 left-0 right-0 z-50 flex items-center justify-between px-4 md:px-12 h-14 md:h-20 bg-canvas/80 backdrop-blur-xl border-b border-border-weak">
        <div className="flex items-center gap-2">
          <span className="font-serif-title text-xl md:text-3xl font-bold tracking-tight">Smart<span className="text-accent">Warga</span></span>
        </div>
        <div className="flex items-center gap-2 md:gap-6">
          <button
            onClick={() => setTheme(prev => prev === "light" ? "dark" : "light")}
            className="p-2 md:p-2.5 rounded-xl bg-surface/50 border border-border-weak text-text-main hover:bg-surface transition-all cursor-pointer flex items-center justify-center"
            title={theme === "light" ? "Ganti ke Mode Gelap" : "Ganti ke Mode Terang"}
          >
            {theme === "light" ? <Moon size={16} className="text-accent" /> : <Sun size={16} className="text-accent-light" />}
          </button>
          <button
          ...
          <button
            onClick={onStart}
            className="bg-accent text-white px-3 py-1.5 md:px-6 md:py-2.5 rounded-xl font-bold text-xs md:text-sm shadow-lg shadow-accent/20 hover:scale-105 active:scale-95 transition-all cursor-pointer"
          >
            Daftar
          </button>
        </div>
        ...
        <div className="max-w-7xl mx-auto grid lg:grid-cols-2 gap-12 items-center">
          ...
          >
            ...
            <div className="flex flex-wrap gap-3">
              <button
                onClick={onStart}
                className="bg-accent text-white px-6 py-3 md:px-10 md:py-5 rounded-xl md:rounded-2xl font-bold text-base md:text-lg flex items-center gap-2 md:gap-3 shadow-xl shadow-accent/20 hover:bg-accent-light hover:-translate-y-1 transition-all cursor-pointer"
              >
                <Zap size={18} className="md:hidden" />
            ...
            <div className="mt-8 md:mt-20 flex gap-6 md:gap-16 border-t border-border-weak pt-6 md:pt-10">
              ...
              <div>
                <div className="font-display text-xl md:text-3xl font-bold mb-0.5 md:mb-1 tracking-tight">24<span className="text-accent">/7</span></div>
                <div className="text-[9px] md:text-xs uppercase font-bold tracking-widest text-text-muted">Akses Layanan</div>
              </div>
              <div>
                <div className="font-display text-xl md:text-3xl font-bold mb-0.5 md:mb-1 tracking-tight">0<span className="text-accent"> Antri</span></div>
                <div className="text-[9px] md:text-xs uppercase font-bold tracking-widest text-text-muted">Proses Digital</div>
              </div>
            </div>
          </motion.div>
          ...
          >
            <div className="relative z-10 bg-surface/30 backdrop-blur-sm border border-border-strong rounded-[2.5rem] p-8 shadow-2xl">
      ...
      <section className="py-12 md:py-32 bg-sidebar relative overflow-hidden">
        <div className="max-w-7xl mx-auto px-5 md:px-12">
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-0.5 bg-border-weak border border-border-weak rounded-2xl md:rounded-[2rem] overflow-hidden">
            <FeatureCard
              icon={<Camera />}
      ...
      <section className="py-14 md:py-40 px-5 md:px-12 text-center relative border-t border-border-weak">
        <div className="max-w-3xl mx-auto">
          ...
          <button
            onClick={onStart}
            className="bg-accent text-white px-8 py-3 md:px-12 md:py-5 rounded-xl md:rounded-2xl font-bold text-base md:text-lg shadow-2xl shadow-accent/20 hover:bg-accent-light hover:scale-105 transition-all cursor-pointer"
          >
            Daftar — Gratis
          </button>
        </div>
      ...
      {/* Footer */}
      <footer className="py-8 md:py-12 border-t border-border-weak px-5 md:px-12">
